...
@WebServlet(
		description = "사용자가 웹에서 *.tam 를 했을 경우 이 서블릿이 응답을 해주도록 한다.", 
		urlPatterns = { "*.tam" }, 
		initParams = { 
				@WebInitParam(name = "propertyConfig", value = "C:\\NCS\\workspace_jsp\\tempSemi\\src\\main\\webapp\\WEB-INF\\Command.properties", description = "*.tam 에 대한 클래스의 매핑파일")
		})
  ...
  */ 
	...
	//	System.out.println("~~~ 확인용 => 서블릿 FrontController 의 init(ServletConfig config) 메소드가 실행됨.");
		...
		try {
			...
			// fis 는 C:/NCS/workspace_jsp/tempSemi/src/main/webapp/WEB-INF/jy/Command.properties 파일의 용도를 읽어오기 위한 파일이다.			
			...
			pr.load(fis);
		/*
			 pr.load(fis); 은  fis 객체를 사용하여 C:/NCS/workspace_jsp/tempSemi/src/main/webapp/WEB-INF/jy/Command.properties 파일의 내용을 읽어다가 
		...
		*/
		...
		/*
          	pr.keys(); 은
          	C:/NCS/workspace_jsp/tempSemi/src/main/webapp/WEB-INF/jy/Command.properties 파일의 내용물에서 
        ...
        */	
			...
			// 	System.out.println("### 확인용 value => " + pr.getProperty(key));
				...
				*/
				...
				if(className != null) {
					...
					// System.out.println("~~~~ 확인용 obj => " + obj);
			...
			System.out.println(" >>> C:/NCS/workspace_jsp/tempSemi/src/main/webapp/WEB-INF/jy/Command.properties 파일이 없습니다. <<< ");
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
	...
	//	~~~ 확인용 url => // http://localhost:9090/MyMVC/member/idDuplicateCheck.up?userid=leess 와 같이 입력되었더라면
