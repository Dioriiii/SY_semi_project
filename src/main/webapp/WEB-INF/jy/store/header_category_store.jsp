...
<script type="text/javascript" src="<%= ctxPath%>/js/jy/headerTest/headerTest.js"></script>
...
<link rel="icon" href="<%= ctxPath%>/images/jy/fav_tamburins.png">
  ...
  <body id="container" style="overflow-x: hidden">
	...
	<nav class="navbar navbar-expand-lg bg-white navbar-white sticky-top ">
		<a class="logo-svg" href="<%=ctxPath%>/main.tam">
	...
	<%-- 사이드바 시작 --%>
		...
		<div class="side_bar">	
			<div id="mySidepanel" class="sidepanel">
			  ...
			  <a href="<%=ctxPath%>/store/findStore.tam">매장보기</a>
			  ...
			  <c:if test="${empty sessionScope.loginuser}">
			  	<a href="<%=ctxPath%>/mypage/mypage.tam">로그인</a>
			  </c:if>
...
<%-- 카테고리 시작 --%>
	<div id="category_container">
		<ul>
			<c:if test="${ not empty requestScope.cntryList}">
			<c:forEach var="cntryCvo" items="${requestScope.cntryList}">
				<li id="${cntryCvo.cty_code}" class="${cntryCvo.cty_code}">
					<a href="/tempSemi/store/findStore.tam?country=${cntryCvo.cty_code}">
						<c:if test="${requestScope.country eq cntryCvo.cty_code}">
							<span class="category_img_select" id=""><img src="<%= ctxPath%>/images/jy/store/${cntryCvo.cty_img}" style="width:100px; height:100px;"></span>
						</c:if>
						<c:if test="${requestScope.country ne cntryCvo.cty_code}">
							<span class="category_img"><img src="<%= ctxPath%>/images/jy/store/${cntryCvo.cty_img}" style="width:100px; height:100px;"></span>
						</c:if>
						<span class="category_name header_footer_font">${cntryCvo.cty_name}</span>
					</a>
				</li>
			</c:forEach>	
			</c:if>
